...
| yalm    | ^4.1.0  | 4.1.0  | Our own, unchanged since 2017; replaced in 1.3.1 by `lib/log.js` (journald-aware); the core lib should take that over.                                                                                                           |
...
| `media` state (#19)    | `com.webos.media/getForegroundAppInfo` (subscribe)                                   | yes    | —   | **done in 1.3.0** as ; newer firmware only. HA uses it for state.                                                       |
...
| `toast` with icon (#7) | `system.notifications/createToast {message, iconData, iconExtension}`                | —      | yes | **done in 1.3.0**: URL/file or raw base64; (with buttons) still open.                                                   |
